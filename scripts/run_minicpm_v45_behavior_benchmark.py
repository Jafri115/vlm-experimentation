...
class MiniCPMRunner:
    """Adapter for the official MiniCPM-V 4.5 model.chat video interface."""
    ...
    def __init__(self, model_id, load_in_8bit=False, use_flash_attn=False):
        ...
        if load_in_8bit:
            raise RuntimeError(
                "For the first fair benchmark use BF16 MiniCPM-V 4.5. "
                "If BF16 does not fit, use the official int4 checkpoint in a separate run."
            )
    ...
    @staticmethod
    def make_temporal_ids(timestamps, window_start, time_scale=0.1):
        """MiniCPM official video format: List[List[int]], packing=1."""
        out = []
        for ts in timestamps:
            local_sec = max(0.0, float(ts) - float(window_start))
            out.append([int(round(local_sec / time_scale))])
        return out
    ...
    def generate(
        ...
        if frames:
            ...
            temporal_ids = self.make_temporal_ids(
                timestamps,
            ...
            # Text-only path used only as a formatting-repair fallback.
...
def run(args):
    ...
    detections_csv = out_dir / "minicpm_v45_behavior_detections.csv"
    details_jsonl = out_dir / "minicpm_v45_behavior_details.jsonl"
    ...
    print("")
    print("MINICPM-V 4.5 BEHAVIOR DETECTION BENCHMARK")
    print("=" * 48)
    ...
    for pos, row in enumerate(
        ...
        try:
            ...
            for w_no, w in enumerate(windows, start=1):
                frames, frame_times = limit_frames_and_times(
                    w["frames"],
                    w["timestamps"],
...
def build_parser():
    ...
    p.add_argument(
        "--output-dir",
        default="./output/minicpm_v45_behavior_benchmark",
    )
    p.add_argument(
        "--model-id",
