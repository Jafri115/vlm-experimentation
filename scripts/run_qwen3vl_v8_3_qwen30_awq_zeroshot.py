...
class Qwen30Runner(v5.QwenRunner):
    """
    V8.3 backend: PRE-QUANTIZED Qwen3-VL-30B-A3B-Instruct AWQ.

    V8.2 exposed a Transformers 5.15 path-matching problem for this checkpoint:
    the checkpoint says the entire visual tower was NOT quantized, using
    modules_to_not_convert=["visual", "mlp.gate"].  In the instantiated
    Qwen3-VL model the visual tower is nested below "model.visual.*".

    Transformers' current AWQ skip matcher does not treat a bare "visual"
    token as a substring match inside "model.visual.*".  It therefore tried
    to replace vision Linear layers with group-size-128 AWQ layers.  The
    Qwen3-VL vision MLP has a 4304 dimension, which is not divisible by 128,
    producing the V8.2 failure.

    V8.3 keeps the checkpoint unchanged and only expands the runtime skip list
    to include "model.visual" explicitly before model construction.
    """

    @staticmethod
    def _load_patched_config(model_id):
        config = AutoConfig.from_pretrained(model_id)

        qcfg = getattr(config, "quantization_config", None)
        if qcfg is None:
            raise RuntimeError(
                "AWQ checkpoint has no quantization_config in config.json."
            )

        # In Transformers this is normally a plain dict at config-load time.
        if isinstance(qcfg, dict):
            current = list(qcfg.get("modules_to_not_convert") or [])
            for name in ("model.visual", "visual", "mlp.gate"):
                if name not in current:
                    current.append(name)
            qcfg["modules_to_not_convert"] = current
            config.quantization_config = qcfg
            patched = current
        else:
            current = list(getattr(qcfg, "modules_to_not_convert", None) or [])
            for name in ("model.visual", "visual", "mlp.gate"):
                if name not in current:
                    current.append(name)
            qcfg.modules_to_not_convert = current
        ...
        print(
            "AWQ runtime modules_to_not_convert:",
            patched,
            flush=True,
        )
        return config
    ...
    def __init__(self, model_id):
        ...
        if not torch.cuda.is_available():
            raise RuntimeError(
                "V8.3 expects an NVIDIA CUDA GPU. CUDA is not available."
            )

        if importlib.util.find_spec("gptqmodel") is None:
            raise RuntimeError(
                "GPTQModel is not installed in this venv."
            )
        ...
        print("Checkpoint: pre-quantized AWQ 4-bit", flush=True)
        print("Vision tower: kept FP16 as checkpoint intended", flush=True)
        ...
        print("Device placement: CUDA:0", flush=True)
        print("Runtime BF16->4bit conversion: NO", flush=True)

        self.processor = AutoProcessor.from_pretrained(model_id)
        config = self._load_patched_config(model_id)
        ...
        if hasattr(self.model, "hf_device_map"):
            print(
                f"hf_device_map: {self.model.hf_device_map}",
                flush=True,
            )
...
def run(args):
    ...
    print("", flush=True)
    print("V8.3 — QWEN3-VL-30B-A3B AWQ VISUAL-SKIP FIX ZERO-SHOT RUPTURE ABLATION", flush=True)
    print("=" * 49, flush=True)
...
def build_parser():
    ...
    p.add_argument(
        "--output-dir",
        default="./output/qwen3vl_v8_3_qwen30_awq_zeroshot_ablation",
    )
