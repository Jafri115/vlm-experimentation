...
The script supports two supervised targets:

1) soft
   Threshold EACH rater independently at WD_P >= 3 and average:
       both < 3      -> 0.0
       one >= 3      -> 0.5
       both >= 3     -> 1.0

2) hard
   Threshold the mean of the two WD_P ratings:
       mean(WD_P) < 3  -> 0
       mean(WD_P) >= 3 -> 1
    ...
    config.update(
        {
            ...
            "hard_label_definition": (
                "mean(rater1,rater2)>=3"
            ),
            "resolved_pos_weight": (
                pos_weight
            ),
            "checkpoint_metric": (
                "validation hard-label AUPRC"
            ),
        }
    )
