...
def _extra_missing_comma_cleanup(candidate):
    """
    Small, conservative repairs not present in V5:
    - missing comma between adjacent JSON objects in an array
    - missing comma between a completed object/array value and next key

    This does NOT alter observation content.
    """
...
def parse_model_json_robust(raw_text, qwen):
    """
    V6.3.1 parser.

    V5's text-only repair uses max_new_tokens=900. With the larger V6.3
    schema, a repaired JSON object can exceed that budget and become
    truncated/malformed. This wrapper preserves V5 behavior first, then
    retries syntax repair with a larger output budget.

    Returns:
        parsed_dict, repair_method
    """
    try:
        return v5.parse_model_json(raw_text, qwen=qwen)
    except Exception as first_error:
        pass

    candidate = v5.extract_json_candidate(raw_text)

    # First try two extra conservative comma repairs.
    cleaned = _extra_missing_comma_cleanup(candidate)
    try:
        return json.loads(cleaned), "extra_deterministic_repair"
    except Exception:
        pass

    # V6.3 JSON can be substantially longer than V5. Give the syntax-only
    # repair call enough tokens to reproduce the whole object.
    repair_raw, _, _, _ = qwen.generate(
        v5.json_repair_messages(candidate),
        max_new_tokens=2400,
    )

    repaired_candidate = v5.extract_json_candidate(repair_raw)
    repaired_candidate = _extra_missing_comma_cleanup(
        repaired_candidate
    )
    ...
    try:
        return json.loads(
            repaired_candidate
        ), "qwen_text_repair_2400"
    except Exception as second_error:
        # Last attempt: ask for a compact syntax-preserving reconstruction.
        compact_prompt = f"""
Convert the malformed JSON below into VALID COMPACT JSON.

STRICT RULES:
- Return JSON only.
- Preserve the same observations and timestamps.
- Do not invent any new visual observation.
- Use exactly these top-level keys:
  window, visibility, states, events, unresolved
- Each state/event may contain only:
  type, start_sec, end_sec, certainty, description
- Remove duplicate entries if needed to make the JSON shorter.
- If one malformed entry cannot be recovered, omit only that entry and
  add "one malformed entry omitted during JSON recovery" to unresolved.
- No markdown.
...
""".strip()
        ...
        compact_raw, _, _, _ = qwen.generate(
            messages,
            max_new_tokens=1800,
        )

        compact_candidate = v5.extract_json_candidate(
            compact_raw
        )
        compact_candidate = _extra_missing_comma_cleanup(
            compact_candidate
        )
        ...
        try:
            return json.loads(
                compact_candidate
            ), "qwen_compact_repair_1800"
        except Exception:
            # Surface the original parsing problem so the caller can save
            # a failure artifact and/or continue gracefully.
            raise second_error
...
def run(args):
    ...
    print("Visual Experiment V6.3.1", flush=True)
    ...
    for pos, row in enumerate(
        ...
        try:
            ...
            for window_no, window in enumerate(
                ...
                try:
                    parsed, repair_method = parse_model_json_robust(
                        raw,
                        qwen=qwen,
                    )
                except Exception as parse_exc:
                    failure_dir = output_dir / "json_failures"
                    failure_dir.mkdir(
                        parents=True,
                        exist_ok=True,
                    )
                    failure_path = (
                        failure_dir
                        / (
                            f"segment_{segment_idx:03d}_"
                            f"window_{window_no:02d}_raw.txt"
                        )
                    )
                    ...
                    raise RuntimeError(
                        "Could not parse observer JSON for "
                        f"segment {segment_idx}, window "
                        f"{window_no}. Raw output saved to "
                        f"{failure_path}"
                    ) from parse_exc
