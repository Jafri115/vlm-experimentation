...
- In evidence/reason fields, prefer literal descriptions such as head orientation,
  gaze direction, posture, arm position, hand movement, facial action, and visible
  mouth movement.
- Do not use "engaged", "attentive", "cooperative", "comfortable", "defensive",
  "avoidant", or similar interpretive words in evidence/reason fields.
...
def run(args):
    ...
    csv_path = (
        output_dir
        / "visual_baseline_predictions.csv"
    )
    jsonl_path = (
        output_dir
        / "visual_baseline_details.jsonl"
    )
...
def build_parser():
    ...
    parser.add_argument(
        "--output-dir",
        default="./output/qwen3vl_visual_baseline_100",
    )
