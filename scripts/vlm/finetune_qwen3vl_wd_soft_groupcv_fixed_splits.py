#!/usr/bin/env python
"""
Qwen3-VL WD_P soft-label patient-grouped cross-validation
=============================================================
...
from vlm import finetune_qwen3vl_wd_soft_consensus_eval as exp
...
def load_fixed_outer_folds(
    manifest: pd.DataFrame,
    split_source_dir: Path,
) -> Dict[str, int]:
    """Reuse outer patient assignments from an earlier CV run."""
    path = split_source_dir / "cv_patient_assignments.csv"
    if not path.exists():
        raise FileNotFoundError(
            f"Fixed-split source is missing {path}"
        )
    ...
    current_patients = set(manifest["patient_id"].astype(str).unique())
    source_patients = set(mapping)
    missing_patients = sorted(current_patients - source_patients)
    if missing_patients:
        raise RuntimeError(
            "Current manifest has patients not present in fixed split source: "
            + ", ".join(missing_patients)
        )

    return {p: mapping[p] for p in sorted(current_patients)}


def assign_fold_splits_from_source(
    manifest: pd.DataFrame,
    split_source_dir: Path,
    outer_fold: int,
) -> Tuple[pd.DataFrame, List[str], List[str], List[str]]:
    """Reuse exact train/validation/test patient lists from an earlier run."""
    path = split_source_dir / f"fold_{outer_fold}" / "patient_split.json"
    if not path.exists():
        raise FileNotFoundError(
            f"Fixed-split source is missing {path}"
        )
    ...
    train_patients = sorted(str(x) for x in spec["train_patients"])
    val_patients = sorted(str(x) for x in spec["val_patients"])
    test_patients = sorted(str(x) for x in spec["test_patients"])

    current_patients = set(manifest["patient_id"].astype(str).unique())
    planned = set(train_patients) | set(val_patients) | set(test_patients)

    if current_patients != planned:
        missing = sorted(current_patients - planned)
        extra = sorted(planned - current_patients)
        raise RuntimeError(
            "Fixed patient split does not match current manifest. "
            f"Missing from source={missing}; absent from current manifest={extra}"
        )
    ...
    if (
        ...
        raise RuntimeError("Patient leakage detected in fixed CV split.")

    fold_manifest = manifest.copy()
    fold_manifest["split"] = "train"
    patient_str = fold_manifest["patient_id"].astype(str)
    fold_manifest.loc[patient_str.isin(val_patients), "split"] = "val"
    fold_manifest.loc[patient_str.isin(test_patients), "split"] = "test"

    return fold_manifest, train_patients, val_patients, test_patients
...
def make_parser():
    ...
    p.description = (
        "Qwen3-VL soft-label WD_P patient-grouped cross-validation with optional fixed patient splits; "
        "train on 0/0.5/1 and evaluate primarily on consensus 0/1 rows."
    )
    ...
    p.add_argument(
        ...
        default="",
        help=(
            "Optional previous CV output directory whose exact outer and "
            "inner patient splits should be reused. This is recommended for "
            "clean threshold ablations such as WD_P >=2 versus WD_P >=3."
        ),
    )
...
def main():
    ...
    if args.target_mode != "soft":
        raise ValueError(
            "This CV experiment is intentionally soft-label training. "
            "Use --target-mode soft."
        )
    ...
    print("Training labels: 0 / 0.5 / 1 (binary rater disagreement retained)")
    print("Primary checkpoint/test metrics: consensus-only 0/1 rows")
    print(
        "NOTE: original manifest train/val/test labels are ignored "
        "for cross-validation assignment."
    )

    # Build two-rater targets once. Soft 0.5 disagreement rows are retained. The helper also preserves all manifest
    ...
    split_source_dir = (
        ...
        if str(args.split_source_dir).strip()
        else None
    )

    if split_source_dir is not None:
        print("Fixed patient splits from:", split_source_dir)
        patient_to_outer_fold = load_fixed_outer_folds(
            manifest=manifest,
    ...
    for fold in range(1, int(args.cv_folds) + 1):
        (
            ...
            assign_fold_splits_from_source(
                manifest=manifest,
                split_source_dir=split_source_dir,
                outer_fold=fold,
            )
            if split_source_dir is not None
            else assign_fold_splits(
                manifest=manifest,
    ...
    # exp.train uses ALL patient-grouped rows for soft-label training; consensus-only rows are used for checkpoint/threshold selection.
