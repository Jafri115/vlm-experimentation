...
def evaluate_wd(prediction_path, ground_truth, output):
    ...
    bad_labels = sorted(
        {row["primary_label"] for row in successful.values()} - LABELS
    )
    if bad_labels:
        raise ValueError(
            f"Unexpected WD-only prediction labels: {bad_labels}"
        )
    ...
    if unknown:
        raise ValueError(
            "Predictions do not match the ground-truth inventory: "
            f"{unknown[:10]}"
        )
    ...
    for uid, prediction in successful.items():
        ...
        try:
            ...
            raise ValueError(
                f"Invalid wd_p_score for {uid}: "
                f"{prediction['wd_p_score']!r}"
            ) from None

        if (
            str(score) != str(prediction["wd_p_score"]).strip()
        ...
        row = {
            ...
            **{
            ...
            "correct_wd": int(
                pred_wd
                == int(truth[uid]["WD_P_binary_mean_gt1"])
            ),
        }
        joined.append(row)
    ...
    write_csv(
        output / "matched_predictions_ground_truth.csv",
        joined,
    )
    write_csv(
        ...
        [
            row
            for row in joined
            if int(row["WD_P_binary_mean_gt1"])
            and not row["pred_wd"]
        ],
    )
    write_csv(
        ...
        [
            row
            for row in joined
            if not int(row["WD_P_binary_mean_gt1"])
            and row["pred_wd"]
        ],
    )
    ...
    strict_rows = [
        row
        for row in joined
        if _is_true(row["strict_wd_two_plus_rater_consensus"])
    ]
    ...
    for provider in sorted(
        {row["transcript_provider"] for row in joined}
    ):
        subset = [
            row
            for row in joined
            if row["transcript_provider"] == provider
        ]
        providers.append(
            {
    ...
    write_csv(
        output / "metrics_by_transcript_provider.csv",
        providers,
    )
    ...
    cohorts = [
        ("all", joined),
        ("strict_wd_consensus", strict_rows),
    ]
    cohorts.extend(
        (
            f"provider:{provider}",
    ...
    for cohort, rows in cohorts:
        truth_field = (
            "strict_wd_binary"
            if cohort == "strict_wd_consensus"
            else "WD_P_binary_mean_gt1"
        )

        for threshold in range(2, 6):
            prediction_field = (
                f"pred_wd_score_ge_{threshold}"
            )

            scored_rows = [
                {
                    **row,
                    prediction_field: int(
                        row["wd_p_score"] >= threshold
                    ),
                }
                for row in rows
            ]
            ...
            threshold_sweep.append(
                {
                    "cohort": cohort,
                    "positive_if_score_at_least": threshold,
                    **binary_metrics(
                        scored_rows,
                        truth_field,
                        prediction_field,
    ...
    write_csv(
        output / "score_threshold_sweep.csv",
        threshold_sweep,
    )
    ...
    write_csv(
        output / "failed_prediction_rows.csv",
        failed,
        list(predictions[0]),
    )
    ...
    summary = {
        ...
        "prediction_label_counts": dict(
            Counter(row["primary_label"] for row in joined)
        ),
        "prediction_score_counts": dict(
        ...
        "threshold_selection_note": (
            "Thresholds are evaluated on this labeled cohort; "
            "choose a final threshold on separate validation data "
            "to avoid optimistic test performance."
        ),
    }
...
def main(args):
    ground_truth, missing = build_ground_truth(
        args.annotations,
        args.inventory,
    )
    ...
    write_csv(
        args.output / "ground_truth_patient_withdrawal.csv",
        ground_truth,
    )
    write_csv(
        args.output / "ground_truth_unmatched_inventory.csv",
        missing,
        ["segment_uid", "reason"],
    )

    print(
        f"Ground truth: {len(ground_truth)} rows; "
        f"unmatched inventory: {len(missing)}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=(
            ...
            / "facs-openface-tools"
            / "data"
            / "completed_segments_merged (1).csv"
        ),
    )
    parser.add_argument(
        "--inventory",
    ...
    parser.add_argument(
        ...
        default=(
            ROOT
            / "output"
            / "qwen3_8b_wd_3rs_ground_truth_comparison"
        ),
    )
