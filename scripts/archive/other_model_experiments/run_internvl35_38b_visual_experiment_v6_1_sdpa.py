...
class InternVLRunner:
    def __init__(
        ...
        # InternVL custom code falls back to eager attention when flash-attn
        # is unavailable. With ~30 frames this materializes a very large
        # attention matrix. Force the Qwen3 language backbone to PyTorch SDPA
        # after model construction so the same visual input can be retained.
        ...
        if attention_backend:
            ...
            if hasattr(self.model, "language_model"):
                ...
                if hasattr(lm, "set_attn_implementation"):
                    lm.set_attn_implementation(attention_backend)
                    changed = True
                elif hasattr(lm, "config"):
                    lm.config._attn_implementation = attention_backend
                    changed = True

            if not changed:
                raise RuntimeError(
                    f"Could not set language-model attention backend to "
                    f"{attention_backend!r}."
                )

            print(
                "Language-model attention backend: "
                f"{getattr(self.model.language_model.config, '_attn_implementation', 'unknown')}",
                flush=True,
            )
...
def run(args):
    ...
    for pos, row in enumerate(
        ...
        try:
            ...
            detail = {
                "experiment": "V6.1 InternVL3.5-38B 4-bit perception benchmark (SDPA)",
...
def build_parser():
    ...
    parser.add_argument(
        ...
        default="sdpa",
        help=(
            "Attention backend for the Qwen3 language backbone. "
            "SDPA is the default because eager attention can OOM "
            "with many visual tokens."
        ),
    )
