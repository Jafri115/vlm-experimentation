#!/usr/bin/env python
"""
V7 — VLM-only zero-shot rupture classification ablation
========================================================
...
Three zero-shot conditions are run with the SAME Qwen3-VL-8B model:
...
def run(args):
    ...
    print("", flush=True)
    print("V7 — VLM-ONLY ZERO-SHOT RUPTURE ABLATION", flush=True)
    print("=" * 49, flush=True)
...
def build_parser():
    p = argparse.ArgumentParser(
        description=(
            "V7: Qwen3-VL-8B VLM-only zero-shot rupture ablation "
    ...
    p.add_argument(
        "--output-dir",
        default="./output/qwen3vl_v7_zeroshot_ablation",
    )
