...
class Qwen30Runner(v5.QwenRunner):
    ...
    V8.1 forces the entire NF4-quantized model onto GPU 0.

    Why: with device_map="auto" + max_memory={0: "24GiB", "cpu": ...},
    Accelerate tried to dispatch some modules to CPU. Transformers' 4-bit
    bitsandbytes validator then rejected that mixed CPU/GPU map before model
    loading finished. A 30B model in NF4 should be substantially smaller than
    the original BF16 checkpoint, so on a 31.84-GiB RTX 5090 the first clean
    test is to keep every quantized module on GPU and let the video settings
    determine whether runtime activation memory fits.
    """
    ...
    def __init__(self, model_id):
        ...
        print("Device placement: ALL model modules -> CUDA:0", flush=True)
        print("CPU offload: disabled for the NF4 load test", flush=True)
        ...
        # IMPORTANT V8.1 FIX:
        # Do not use device_map="auto" with a 24-GiB cap here. That cap caused
        # Accelerate to place part of the 4-bit model on CPU, and Transformers
        # rejected the mixed map before loading. Force the whole quantized
        # model onto the 5090 first.
...
def run(args):
    ...
    print("", flush=True)
    print("V8.1 — QWEN3-VL-30B-A3B 4-BIT ALL-GPU ZERO-SHOT RUPTURE ABLATION", flush=True)
    print("=" * 49, flush=True)
