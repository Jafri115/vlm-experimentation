...
class InternVLRunner:
    ...
    def generate(self, prompt, frames, max_new_tokens=1100):
...
{{
  ...
  "states": [
    {{
      ...
      "end_sec": {min(window_start + 2.0, window_end):.1f},
      "certainty": "clear|possible|ambiguous",
...
Only return behavior that is visually present.
""".strip()
    ...
    for pos, row in enumerate(
        ...
        try:
            ...
            for w_no, w in enumerate(windows, start=1):
                ...
                try:
                    parsed, parse_method = extract_json(raw, model=model)
                except Exception as parse_exc:
                    failure_dir = out_dir / "json_failures"
                    ...
                    raise RuntimeError(
                        f"Could not recover JSON for segment {idx}, "
...
def build_parser():
    ...
    p.add_argument(
        "--output-dir",
        default="./output/internvl3_8b_behavior_benchmark",
    )
    p.add_argument(
        "--model-id",
