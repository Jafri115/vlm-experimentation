...
def train(
    ...
    for epoch in range(
        ...
        # Each patient has approximately equal expected contribution per epoch,
        # regardless of how many minute segments that patient contributes.
        # Epoch length remains identical to len(train_df).
        patient_counts = (
            train_df["patient_id"]
            .astype(str)
            .value_counts()
        ...
        sampling_weights = (
            train_df["patient_id"]
            .astype(str)
            .map(lambda pid: 1.0 / float(patient_counts.loc[pid]))
        ...
        sampled_patient_counts = (
            epoch_df["patient_id"]
            .astype(str)
            .value_counts()
            .sort_index()
        )
        print("Patient-balanced epoch sampling counts:")
        print(sampled_patient_counts.to_string())
        ...
        if args.target_mode == "consensus":
            ...
            if not threshold_rows:
                raise RuntimeError(
                    "Consensus mode has no consensus-only validation rows."
                )
