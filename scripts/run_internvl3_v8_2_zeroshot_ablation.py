#!/usr/bin/env python
"""
InternVL3-8B — VLM-only zero-shot rupture classification ablation
==================================================================
...
Three zero-shot conditions are run with the SAME InternVL3-8B model:
...
B) WINDOWED_DIRECT
   ...
   -> same InternVL model aggregates the assessments into a segment decision.

C) DESCRIBE_JUDGE
   Four independent 15-s windows -> literal visual descriptions ONLY
   -> same InternVL model receives only the descriptions + compact visual 3RS
   rubric -> segment rupture decision.
...
if not hasattr(transformers, "Qwen3VLForConditionalGeneration"):
    class _UnavailableQwen3VLForConditionalGeneration:
        ...
        def from_pretrained(cls, *args, **kwargs):
            raise RuntimeError(
                "Qwen3-VL is unavailable in the InternVL environment. "
                "This runner uses V5 only for video and face-track utilities."
            )
...
def limit_frames_evenly(frames, max_frames):
    """Keep evenly spaced frames without changing their temporal order."""
    frames = list(frames)
...
def limit_frames_and_times(frames, timestamps, max_frames):
    ...
    if max_frames == 1:
        index = len(frames) // 2
        return [frames[index]], [timestamps[index]]
...
class InternVLRunner:
    """Small adapter exposing the generate interface used by this experiment."""
    ...
    def __init__(
        ...
        if transformers.__version__ != SUPPORTED_TRANSFORMERS:
            raise RuntimeError(
                "InternVL3 custom model code is pinned to transformers=="
                f"{SUPPORTED_TRANSFORMERS}, but {transformers.__version__} "
                "is installed. Run: python -m pip install --force-reinstall "
                f"\"transformers=={SUPPORTED_TRANSFORMERS}\""
            )

        if not torch.cuda.is_available():
            raise RuntimeError("InternVL3-8B inference requires a CUDA GPU.")

        self.model_id = model_id
        self.dtype = torch.bfloat16
        self.device = torch.device("cuda:0")
        self.transform = build_internvl_transform(input_size=448)
        ...
        for message in messages:
            ...
            for item in content:
                ...
                if item_type == "text":
                    ...
                    frames = list(item.get("video", []))

        return "\n".join(prompt_parts).strip(), frames

    def generate(self, messages, max_new_tokens=800):
        prompt, frames = self._unpack_messages(messages)
        pixel_values = None
        ...
        del pixel_values
        return str(response), elapsed, peak_allocated, peak_reserved
...
def parse_json_robust(raw_text, vlm, max_repair_tokens=1600):
    """
    Strict -> V5 deterministic repair -> extra comma repair ->
    InternVL syntax-only repair.

    The repair call uses the SAME InternVL model and is formatting-only.
    """
...
""".strip()
    ...
    repaired_raw, _, _, _ = vlm.generate(
        messages,
    ...
    try:
        ...
        repaired = str(repaired_raw).strip()
...
def binary_metrics(y_true, y_pred):
    ...
    return {
        ...
        "specificity": (
            round(specificity, 4) if specificity is not None else None
        ),
        "f1": round(f1, 4),
    }
...
def run(args):
    ...
    predictions_csv = output_dir / "internvl3_condition_predictions.csv"
    summary_csv = output_dir / "internvl3_segment_summary.csv"
    details_jsonl = output_dir / "internvl3_details.jsonl"
    ...
    print("", flush=True)
    print("INTERNVL3-8B — VLM-ONLY ZERO-SHOT RUPTURE ABLATION", flush=True)
    print("=" * 57, flush=True)
    ...
    print(
        ...
        f"capped at {args.direct_max_frames} frames",
        flush=True,
    )
    print(
        f"Each 15-s window is capped at {args.window_max_frames} frames",
        flush=True,
    )
        ...
        try:
            ...
            if "direct" in remaining:
                ...
                try:
                    ...
                    direct_frames, direct_times = limit_frames_and_times(
                        direct_frames,
                        direct_times,
                        args.direct_max_frames,
                    )
            ...
            if "windowed_direct" in remaining:
                ...
                try:
                    for w_no, w in enumerate(windows, start=1):
                        window_frames = limit_frames_evenly(
                            w["frames"],
                    ...
                    final_raw, final_sec, pa, pr = vlm.generate(
    ...
    print("\nInternVL3 experiment finished.", flush=True)
...
def build_parser():
    p = argparse.ArgumentParser(
        description=(
            "V8.2: InternVL3-8B VLM-only zero-shot rupture ablation "
    ...
    p.add_argument(
        "--output-dir",
        default="./output/internvl3_v8_2_zeroshot_ablation",
    )
    ...
    p.add_argument(
        "--use-flash-attn",
        action="store_true",
        help="Use FlashAttention when it is installed and supported.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=16,
        help="Uniform frame cap for each 15-second InternVL call.",
    )
    ...
    p.add_argument(
        ...
        type=int,
        default=24,
        help="Uniform frame cap for the full-segment InternVL call.",
    )
