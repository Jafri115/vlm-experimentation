...
class Qwen30Runner(v5.QwenRunner):
    """
    V8.2 backend: PRE-QUANTIZED Qwen3-VL-30B-A3B-Instruct AWQ.

    This avoids the previous 62-GB BF16 -> runtime-NF4 conversion path.
    The Hugging Face checkpoint is already 4-bit AWQ, so Transformers loads
    the quantized weights directly.

    The inherited V5 generate() method remains unchanged. Therefore the
    scientific V7 conditions/prompts/video preparation are unchanged.
    """
    ...
    def __init__(self, model_id):
        ...
        if not torch.cuda.is_available():
            raise RuntimeError(
                "V8.2 expects an NVIDIA CUDA GPU. CUDA is not available."
            )

        # Current Transformers AWQ integration requires gptqmodel.
        if importlib.util.find_spec("gptqmodel") is None:
            raise RuntimeError(
                "The current Transformers AWQ loader requires GPTQModel, "
                "but 'gptqmodel' is not installed in this venv. Run:\n"
                r".\.venv\Scripts\python.exe -m pip install -U gptqmodel"
            )
        ...
        print(f"Compute dtype for non-quantized weights: {compute_dtype}", flush=True)
        ...
        # The model's own config.json contains quantization_config=AWQ.
        # Do NOT pass BitsAndBytesConfig here; that would attempt a second
        # quantization pass over an already-quantized checkpoint.
        self.model = (
            Qwen3VLMoeForConditionalGeneration
            .from_pretrained(
                model_id,
...
def run(args):
    ...
    print("", flush=True)
    print("V8.2 — QWEN3-VL-30B-A3B AWQ ZERO-SHOT RUPTURE ABLATION", flush=True)
    print("=" * 49, flush=True)
...
def build_parser():
    ...
    p.add_argument(
        "--output-dir",
        default="./output/qwen3vl_v8_2_qwen30_awq_zeroshot_ablation",
    )
